...
Dán workflow hoặc link file: [02-group-problem-statement-workflow.md](02-group-problem-statement-workflow.md)
...
```mermaid
flowchart LR
    A[Tải BCTC PDF] --> B[Tìm bảng chính]
    B --> C[Nhập số liệu vào Excel]
    C --> D[Kiểm tra số học]
    D --> E[Tính chỉ số]
    E --> F[Viết nhận định]
    F --> G[Review nguồn]
...
```mermaid
flowchart LR
    A[Upload BCTC PDF] --> B[Preflight]
    B --> C[AI trích xuất số liệu]
    C --> D{Validate 0 ERROR?}
    D -->|Không| E[Đọc lại PDF và sửa]
    E --> C
    D -->|Có| F[Tính chỉ số]
    F --> G[AI draft báo cáo]
    G --> H[Người phân tích review]
```
